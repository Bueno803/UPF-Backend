...
export class UsersController {
  ...
  async createAuthUser(
    @Res() response,
    @Body() authUserDto: ICreatePassengerAuthModel,
  ): Promise<PassengerAuth> {
    // To-Do
    ...
    try {
      ...
      await this.usersService
        .createUserWithEmailPassword(email, phoneNumber, password)
        .then((resData) => {
  ...
  @Post('/loggin-update')
  async userLogginTimeUpdate(
    @Res() response,
    @Body() userDto: { uid: string; lastLoggedIn: Date },
  ): Promise<Passenger> {
    // To-Do
    ...
    try {
      ...
      const user = await this.usersService.updateUserTimeStamp(
        uid,
        lastLoggedIn,
      );
      console.log(user);
  ...
  @Post('/update-phone')
  async userPhoneUpdate(
    @Res() response,
    @Body() userDto: { uid: string; phoneNumber: string },
  ): Promise<Passenger> {
    // To-Do
