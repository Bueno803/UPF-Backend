...
export class ClassInProgressService {
  ...
  async getClassStamps() {
    try {
      const response = [];
      let TechInfo;
      const docRef = await admin.firestore().collection('last_class_time_stamp');
      await docRef.get().then(async (result) => {
        result.forEach((doc) => {
          response.push(doc.data());
        });
      });
         TechInfo = { lilInfo: await this.getLilTechInfo(),
  ...
  async getLilTechInfo() {
    try {
      ...
      const kicksResponse = [];
      const formsDocRef = await admin.firestore().collection('lil_formstid_info');
      const handAtksDocRef = await admin.firestore().collection('lil_handatkstid_info');
      const blocksDocRef = await admin.firestore().collection('lil_blockstid_info');
      const kicksDocRef = await admin.firestore().collection('lil_kickstid_info');
      ...
      return {forms: formsResponse, handAtks: handAtksResponse, blocks: blocksResponse, kicks: kicksResponse};
    } catch (error) {
      console.log('An error occurred fetching class stamps: ', error);
      return error;
    }
  ...
  async getAdvTechInfo() {
    try {
      ...
      const blocksResponse = [];
      const kicksResponse = [];
      const formsDocRef = await admin.firestore().collection('adv_formstid_info');
      const handAtksDocRef = await admin.firestore().collection('adv_handatkstid_info');
      const blocksDocRef = await admin.firestore().collection('adv_blockstid_info');
      const kicksDocRef = await admin.firestore().collection('adv_kickstid_info');
